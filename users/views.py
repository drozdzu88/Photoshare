...
def cancel_friend_request(request, id):
	user = get_object_or_404(User, id=id)
	frequest = FriendRequest.objects.filter(from_user=request.user, to_user=
		user).first()
...
def profile_view(request, slug):
	...
	# Sprawdzamy czy to użytkownik czy przyjaciel
	button_status = 'none'
	if p not in request.user.profile.friends.all():
		button_status = 'not_friend'

		# Jeżeli wysłaliśmy mu zaproszenie do znajomych
		if len(FriendRequest.objects.filter(from_user=request.user).filter(to_user=p.user)) == 1:
			button_status = 'friend_request_sent'

		# Jeżeli otrzymaliśmy zaproszenie do przyjaciół
...
def register(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			messages.success(request, f'Twoje konto zostało utworzone. Możesz teraz się zalogować!')
...
def my_profile(request):
	...
	# Czy ten użytkownik jest naszym przyjacielem
	button_status = 'none'
	if p not in request.user.profile.friends.all():
		button_status = 'not_friend'


		# Jeżeli wysłaliśmy mu zaproszenie
		...
		if len(FriendRequest.objects.filter(from_user=you).filter(to_user=request.user)) == 1:
			button_status = 'friend_request_recived'
